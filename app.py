...
@st.cache_resource
def run_training_v45(_detector, df):
    """모델 학습 (별도 캐시)"""
...
# 모델 및 데이터 로드
detector = load_detector_v45()
if detector is None:
    st.stop()
df = load_data_v45(detector)

if df is None:
    st.error("데이터 로드 실패: 데이터를 가져올 수 없습니다. (소스 데이터 오류 또는 API 문제)")
    st.stop()

detector = run_training_v45(detector, df)
