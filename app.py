...
@st.cache_resource
def run_training_v24(_detector, df):
    """모델 학습 (별도 캐시)"""
...
# 모델 및 데이터 로드
detector = load_detector_v24()
if detector is None:
    st.stop()
df = load_data_v24(detector)
detector = run_training_v24(detector, df)
