...
@st.cache_resource
def run_training_v46(_detector, df):
    """모델 학습 (별도 캐시)"""
...
# 모델 및 데이터 로드
detector = load_detector_v46()
if detector is None:
    st.stop()
df = load_data_v46(detector)

if df is None:
    st.error("데이터 로드 실패: 데이터를 가져올 수 없습니다. (소스 데이터 오류 또는 API 문제)")
    st.stop()

detector = run_training_v46(detector, df)
...
# ==========================================
    ...
    available_features = [
        ...
        'context_fx_shock', 'context_vol_shock'
    ]
    selected_features = st.multiselect(
        "분석할 피처 선택",
        available_features,
        default=['hmm_strain', 'context_fx_shock', 'context_vol_shock']
    )
